...
type ValuesUnion<T> = T extends {
    [key: string]: infer U;
} ? U : T extends (infer U)[] ? U : never;

declare const RelayEnvironment: {
    readonly DEV: "DEV";
    readonly PROD: "PROD";
};
type RelayEnvironment = ValuesUnion<typeof RelayEnvironment>;

type RelayClient = ReturnType<typeof createOpenApiClient<paths>>;
declare function createRelayClient({ environment, authKey, }: {
    environment: RelayEnvironment;
    authKey: string;
}): RelayClient;

export { type RelayClient, type components as RelayComponents, RelayEnvironment, type operations as RelayOperations, type paths as RelayPaths, createRelayClient };
